<?php // scripts/pending/2026_06_30_fix_lmcline_bal_inflated_amt_totalpayout_nlio00946.php

/**
 * Fix inflated amt_totalpayout on wip_l_lmcline_bal for NLIO00946.
 *
 * ROOT CAUSE
 * ----------
 * Java SAERP uses encumbrance accounting: it increments
 * wip_l_lmcline_bal.amt_totalpayout at DR-SAVE time (not PR time).
 * For NLIO00946, the Maker drafted in Java offline, which encumbered
 ...
 *   NLMC0008189  PR  ₱5,000.00  → bal_id 38034  VIDEO LIVESTREAMING
 *   NLMC0008190  PR  ₱1,000.00  → bal_id 38030  INCENTIVE FOR EMCEE
 *   NLMC0008191  PR  ₱1,000.00  → bal_id 38033  SINGER
 *   NLMC0008192  PR    ₱800.00  → bal_id 38024  MERIENDA PACKAGE
 *   NLMC0008193  PR  ₱1,500.00  → bal_id 38032  PHOTOGRAPHER AND VIDEOGRAPHER
 *
 * Current state (inflated):
 *   bal_id 38024  MERIENDA PACKAGE               budget ₱800    paid ₱1,600  ← 2×
 *   bal_id 38030  INCENTIVE FOR EMCEE            budget ₱1,000  paid ₱2,000  ← 2×
 *   bal_id 38032  PHOTOGRAPHER AND VIDEOGRAPHER  budget ₱1,500  paid ₱3,000  ← 2×
 *   bal_id 38033  SINGER                         budget ₱1,000  paid ₱2,000  ← 2×
 *   bal_id 38034  VIDEO LIVESTREAMING            budget ₱5,000  paid ₱10,000 ← 2×
 *
...
return function ($cmd) {
    ...
    // Expected scope for all 5 lines — NLIO00946 INTERMENT ORDER scope.
    // Checked per-row to prevent accidental writes to a different IO.
    $EXPECTED_SCOPE_ID = 25956;
    ...
    $corrections = [
        [
            'wip_l_lmcline_bal_id' => 38024,
            'description'          => 'MERIENDA PACKAGE',
            'correct_amt'          => 800.00,
            'pr_documentno'        => 'NLMC0008192',
        ],
        [
            'wip_l_lmcline_bal_id' => 38030,
            'description'          => 'INCENTIVE FOR EMCEE',
            'correct_amt'          => 1000.00,
            'pr_documentno'        => 'NLMC0008190',
        ],
        [
            'wip_l_lmcline_bal_id' => 38032,
            'description'          => 'PHOTOGRAPHER AND VIDEOGRAPHER',
            'correct_amt'          => 1500.00,
            'pr_documentno'        => 'NLMC0008193',
        ],
        [
            'wip_l_lmcline_bal_id' => 38033,
            'description'          => 'SINGER',
            'correct_amt'          => 1000.00,
            'pr_documentno'        => 'NLMC0008191',
        ],
        [
            'wip_l_lmcline_bal_id' => 38034,
            'description'          => 'VIDEO LIVESTREAMING',
            'correct_amt'          => 5000.00,
            'pr_documentno'        => 'NLMC0008189',
        ],
    ];
    ...
    echo "{$line}\n";
    echo "FIX INFLATED amt_totalpayout — NLIO00946\n";
    ...
    foreach ($corrections as $c) {
        ...
        // 2. Scope membership — must belong to NLIO00946 (scope 25956)
    ...
    try {
        ...
        echo "{$line}\n";
        if (isset($cmd)) $cmd->info("nlio00946-inflated-bal: {$totalRows} row(s) updated and verified.");
